...
##  Benchmark: SAFE-I vs Levelup (RCC) vs HEDT

The `benchmark` directory allows you to compare **SAFE-I (SAFE-Boost's Inference)**,**Levelup’s RCC (Reduced Complexity Comparison)** and **HEDT** methods in an encrypted environment.
...
Navigate to each folder (`SAFE-I`, `levelup`,`HEDT`) and run the benchmarks for the **Iris, Breast-Cancer, and Spambase** datasets.

* **Run SAFE-I:** `cd benchmark/SAFE-I && python run.py`
* **Run Levelup:** `cd benchmark/levelup && python run.py`
* **Run HEDT:** `cd benchmark/HEDT && python run.py` (Note: This study focuses on Decision Trees rather than XGBoost, meaning only a single tree can be configured.)
