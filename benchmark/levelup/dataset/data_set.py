...
import numpy as np
from utils import left_rotate_bin, right_rotate_bin
...
class DataList:
    """
    A collection of HEaan-Stat Blocks (a row of data).
    """
    def __init__(
        self,
        context: Context,
    ...
    def save(self, path: Path, target_level: Optional[int] = None):
        ...
        for idx, block in enumerate(self.block_list):
            block.save(path / f"block_{idx}", target_level=target_level)
    def load(self, path: Path, num_blocks: int, encrypted: bool) -> "DataList":
        self.set_block_list([Block.from_path(self.context, path / f"block_{i}", encrypted=encrypted) for i in range(num_blocks)])
        return self
    def decrypt(self):
...
class DataMatrix:
    """
    A collection of DataLists (a matrix where each row is a DataList).
    """
    def __init__(
        self,
        context: Context,
    ...
    def sum_rows(self) -> DataList:
        """Returns a DataList where each element is the sum of its corresponding row's blocks."""
        res = DataList(self.context, self.encrypted)
        res.set_block_list([r.sum_block() for r in self.rows])
        return res
    def sum_matrix(self) -> Block:
        """Sums every single block in the matrix into one Block."""
    ...
    def save(self, path: Union[str, Path], target_level: Optional[int] = None):
        ...
        for i, row in enumerate(self.rows):
            row.save(path / f"row_{i}", target_level=target_level)
    @staticmethod
