        ...
        for nt in ntree:
            for dep in range(2, 9):
                ...
                for _ in range(11):
                    ...
                    ct_check = utils.hedt_check_pred(context, ct_pred=ct_predict)
                    print("ct_check:", ct_check)
                    check_list = [1 if abs(ct_check[i] - plain_pred) <= 10**(-5)   else 0 for i in range(len(ct_check))]
