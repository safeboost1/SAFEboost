...
random.seed(42)
np.random.seed(42)
def dummy_find_max_pos(context, data, node, tree_idx):
    num_slots = context.num_slots
    from dataset.data_set import DataList
    file_path = os.path.join("/home/jachoi/projects/xgboost/he/syn4_dummy_max_pos", f"tree{tree_idx}", f"{node.id}.pkl")
    with open(file_path, "rb") as pf:
        data = pickle.load(pf)  
    chunks = []
    for i in range(0, len(data), num_slots):
        chunk = data[i : i + num_slots]
        if len(chunk) < num_slots:
            pad_length = num_slots - len(chunk)
            chunk.extend([0] * pad_length)
        chunks.append(chunk)
    max_pos_list = DataList(context, True, chunks)
    return max_pos_list
def else_dummy_find_max_pos(context, depth, tree_idx):
    from dataset.data_set import DataList
    num_slots = context.num_slots
    file_path = os.path.join(
        "/home/jachoi/projects/he-xgboost/he/cancer_dummy_max_pos", f"tree{tree_idx}", f"depth{depth}.pkl"
    )
    with open(file_path, "rb") as pf:
        data = pickle.load(pf)  
    chunks = []
...
def decrypt_res_by_one_row(context, model):  
    ...
    for i in range(rows):
        ...
        Fm = 0
        tmp = DataMatrix(context=context).load( row_path, True)
        for dlist in tmp:
            for data in dlist:
                m = data.decrypt()
